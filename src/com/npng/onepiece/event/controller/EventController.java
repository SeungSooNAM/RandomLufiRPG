...
//	private EventService eventService = new EventService();
	...
	public int random(int map) {
		...
		if(map == 1 || map == 3 || map == 4 || map == 5) {
			 number = (int)(Math.random() *38)+3;
		}
		if(map == 2) {
			number = (int)(Math.random() *48) + 2;  //조로
			if(number >= 41) {
				number = 2;
			}
		}
		if(map == 6) {
			number = (int)(Math.random() *48) + 3;  //핸콕
			if(number >= 42) {
				number = 41;
			}
		}
	...
	public int sceresult(int map , int num, int chNum, EventResultView view ) {
		...
		System.out.println("시나리오 번호 : " + num);
		//선택지, 시나리오번호,캐릭터번호
	...
	public int hfriendcheck(int chNum) {   // 6번 조로 있는지 체크  있으면 1반환 없으면 0반환

		int result = eventservice.hfriendcheck(chNum);
	...
	public int updateHankok() {
		...
		int result = eventservice.updateHankok();
