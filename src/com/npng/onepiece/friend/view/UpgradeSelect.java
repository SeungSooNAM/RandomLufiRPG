...
public class UpgradeSelect extends JPanel{
	
	private UpgradeSelect upgradeSelect;
	...
	public UpgradeSelect(String name) {
		this.setSize(1200, 800);
		this.upgradeSelect = this;
		...
		Font font2 = new Font("SansSerif", 0, 18);
		...
		Image imgUpgrade = new ImageIcon("image/friend/강화.png").getImage().getScaledInstance(150, 55, 0);
		...
		Image imgCheck = new ImageIcon("image/friend/텍스트설명.png").getImage().getScaledInstance(400, 200, 0);
		JLabel check = new JLabel(new ImageIcon(imgCheck));
		checkPanel.add(check);
		
		JLabel text = new JLabel("강화 비용 : 50베리. 강화 하시겠습니까?");
		text.setFont(font2);
		text.setForeground(Color.white);
		text.setBounds(20, 0, 400, 30);
		check.add(text);
		
		
		okPanel = new JPanel();
		okPanel.setLocation(35, 120);
		okPanel.setSize(150, 55);
		okPanel.setOpaque(false);
		check.add(okPanel);
		Image imgOk = new ImageIcon("image/friend/예.png").getImage().getScaledInstance(150, 55, 0);
		JLabel ok = new JLabel(new ImageIcon(imgOk));
		okPanel.add(ok);
		
		cancelPanel = new JPanel();
		cancelPanel.setLocation(225, 120);
		cancelPanel.setSize(150, 55);
		cancelPanel.setOpaque(false);
		check.add(cancelPanel);		
		Image imgCancel = new ImageIcon("image/friend/아니오.png").getImage().getScaledInstance(150, 55, 0);
		JLabel cancel = new JLabel(new ImageIcon(imgCancel));
		cancelPanel.add(cancel);
		
		Image text2 = new ImageIcon("image/friend/" + name + "설명.png").getImage().getScaledInstance(600, 160, 0);
		JLabel textLabel = new JLabel(new ImageIcon(text2));
	...
	public class MyMouseAdapter extends MouseAdapter{
		...
		public void mouseClicked(MouseEvent e) {
			if(e.getSource() == okPanel) {
				int cNum = 1;
				int result = friendController.upgrade(name, gold);
				if(result == 2) {
					ViewUtil.changePanel(upgradeSelect, new UpgradeSuccess(name));
				} else if(result == 1 ) {
					ViewUtil.changePanel(upgradeSelect, new UpgradeFailed(name));
				} else if(result == 0){
					ViewUtil.changePanel(upgradeSelect, new NotEnoughMoney(name));
				}

			} else if(e.getSource() == cancelPanel){
				ViewUtil.changePanel(upgradeSelect, new SelectInfo(name));
			}
		}
	}
}
